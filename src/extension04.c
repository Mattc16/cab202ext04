...
void matrix_sum(int16_t *a, uint8_t *dim_a, int16_t *b, int16_t *result)
{
    for (uint8_t z = 0; z < dim_a[0]; z++)
    {
        for (uint8_t x = 0; x < dim_a[1]; x++)
        {
            result[z * dim_a[1] + x] = a[z * dim_a[1] + x] + b[z * dim_a[1] + x];
        }
    }
}

void matrix_add(int16_t *a, uint8_t *dim_a, int16_t addend, int16_t *result)
{
    for (uint8_t z = 0; z < dim_a[0]; z++)
    {
        for (uint8_t x = 0; x < dim_a[1]; x++)
        {
            result[z * dim_a[1] + x] = a[z * dim_a[1] + x] + addend;
        }
    }
}

void matrix_scale(int16_t *a, uint8_t *dim_a, int16_t multiplicand, int16_t *result)
{
    for (uint8_t z = 0; z < dim_a[0]; z++)
    {
        for (uint8_t x = 0; x < dim_a[1]; x++)
        {
            result[z * dim_a[1] + x] = a[z * dim_a[1] + x] * multiplicand;
        }
    }
}

void matrix_mul(int16_t *a, uint8_t *dim_a, int16_t *b, uint8_t *dim_b, int16_t *result)
{
    if (dim_a[1] == dim_b[0])
    {
        for (uint8_t z = 0; z < dim_a[0]; z++)

            for (uint8_t x = 0; x < dim_b[1]; x++)
            {
                result[z * dim_b[1] + x] = 0;
                for (uint8_t k = 0; k < dim_b[0]; k++)
                {
                    result[z * dim_b[1] + x] += a[z * dim_a[1] + k] * b[k * dim_b[1] + x];
                }
            }
    }
}
